...
def default_tranco_crawl(with_blocklist: bool):
    ...
    if with_blocklist:
        browser_params[0].blocklists = ["easylist_general_block.txt"]
    ...
    with TaskManager(
        manager_params,
        browser_params,
        SQLiteStorageProvider(Path("./datadir/tranco_100.sqlite")),
        None,
    ) as manager:
        # Visits the sites
...
default_tranco_crawl(with_blocklist=False)
default_tranco_crawl(with_blocklist=True)
